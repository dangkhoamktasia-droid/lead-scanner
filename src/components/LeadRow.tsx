...
export function LeadRow({ lead, selected, onSelect, onApprove, onReject, onMessageEdit }: LeadRowProps) {
  ...
  const confColor = lead.confidence >= 0.85
  ...
  return (
    <>
      <tr className={`transition-colors ${selected ? 'bg-indigo-50/60' : 'hover:bg-white/60'}`}>
        ...
        <td className="px-4 py-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <button onClick={() => setShowPost(!showPost)} className="text-xs text-gray-400 hover:text-indigo-500 mt-0.5 transition-colors">
                {showPost ? 'Ẩn' : 'Xem bài'}
              </button>
            </div>
          </div>
        ...
        <td className="px-4 py-3">
          <span className="text-xs text-gray-600 bg-gray-100 px-2 py-0.5 rounded-full">{lead.hinhThucCast || '—'}</span>
        </td>
        <td className="px-4 py-3 text-xs text-gray-700 font-medium">
          <div className="max-w-[180px] break-words whitespace-normal leading-relaxed">{lead.sanPhamDichVu || '—'}</div>
        </td>
        <td className="px-4 py-3 text-xs text-gray-600 font-mono">{lead.sdtLienHe || '—'}</td>
        <td className="px-4 py-3">
          <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${confColor}`}>{confidencePct}%</span>
        </td>
          ...
          {editingMsg ? (
            <div className="flex flex-col gap-1">
              ...
              <div className="flex gap-1">
                <button onClick={saveMsg} className="p-1 rounded bg-emerald-100 text-emerald-600 hover:bg-emerald-200">
                  <Check className="w-3 h-3" />
                </button>
                <button onClick={() => setEditingMsg(false)} className="p-1 rounded bg-red-100 text-red-500 hover:bg-red-200">
                  <X className="w-3 h-3" />
                </button>
              </div>
            </div>
            ...
            <div className="flex items-start gap-1 group">
              <p className="text-xs text-gray-600 line-clamp-2 flex-1">{lead.message || <span className="text-gray-300 italic">Chưa có</span>}</p>
        ...
        <td className="px-4 py-3">
          <span className={`text-xs px-2.5 py-1 rounded-full font-medium ${statusBadge[lead.status] ?? 'bg-gray-100 text-gray-500'}`}>
      ...
      {showPost && (
        <tr className={selected ? 'bg-indigo-50/40' : 'bg-gray-50/60'}>
          <td colSpan={9} className="px-6 pb-4 pt-1">
            <div className="text-xs text-gray-600 bg-white/80 border border-gray-100 rounded-xl p-3 max-h-40 overflow-y-auto whitespace-pre-wrap shadow-inner">
              {lead.postText}
            </div>
